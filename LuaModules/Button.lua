...
function M.unhover(button)
    if gui.get_scale(button.buttonNode) == button.scale * 1.1 then -- was button already unhovered?
        -- over hover
        gui.set_scale(button.buttonNode, button.scale)
...
function M.on_input(self, action_id, action, button)
    ...
    if defos.is_mouse_in_view() and gui.is_enabled(parent) and gui.get_visible(button.buttonNode) then -- is cursor inside visible button?
        if (button.inputZone == nil and gui.pick_node(button.buttonNode, action.x, action.y) ) 
            ...
            if not button.wasClicked or action.released then -- was button clicked? if yes, it is blocked for some time 
                ...
                if button.hoverSound ~= nil 
                    and gui.get_scale(button.buttonNode) ~= button.scale * 1.1 then -- is hover sound set and wasn't played?
                    -- play hover sound
                    msg.post(SCENE_MANAGER, "PlaySound", {sound = button.hoverSound})
                end
                
                -- visual hover
                gui.set_scale(button.buttonNode, button.scale * 1.1)
